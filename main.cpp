...
void unpack_color(const uint32_t &color, uint8_t &r, uint8_t &b, uint8_t &g, uint8_t &a) {
    r = (color >> 0) & 255;
    g = (color >> 8) & 255;
    b = (color >> 16) & 255;
    a = (color >> 24) & 255;
...
void drop_ppm_image(const std::string filename, const std::vector <uint32_t> &image, const size_t height, const size_t width) {
    assert(image.size() == width*height); //Makes sure img is product of screen height & width
    std::ofstream ofs;
    ofs.open(filename,std::ofstream::out | std::ofstream::binary); //Issue with windows GCC

    if (!ofs) { std::cout << "Error opening file" << std::endl;
        return;}
    ofs << "P6\n" << width << " "<<  height << "\n255\n"; //Bug here maybe
...
int main() {
    ...
    for (size_t j = 0; j < win_h; j++) { //This loop fills the screen with color
        for (size_t i = 0; i < win_w; i++) {
            uint8_t r = 255*j/float(win_h); //Varies 0 - 255 as j sweeps vertically
            uint8_t g = 255*i/float(win_w); // Varies 0 - 255 as j sweeps horizontally
            uint8_t b = 0;
    ...
    for (size_t j = 0; j < map_h; j++) {
        for (size_t i = 0; i < map_w; i++) {
            if (map[i+j*map_w]==' ') continue; //Skips empty spaces
